/*:
 ...
 # Composite
 ---
 ![](Composite_diagram1.png)
 ![](Composite_diagram2.png)
 
 É um structural design pattern que nos permite compor objetos em uma estrutura de árvore, e então trabalhar nessas estruturas como se fossem objetos individuais.
 
 Esse padrão proporciona tipos básicos de elementos que compartilham a mesma interface: containers (caixas) e leaves (produtos dentro dessas caixas). Um container pode ser comporto por leaves ou outros containers. Com o padrão, podemos tratar os dois tipos de elementos uniformemente, já que todos os elementos compartilham a mesma interface.
